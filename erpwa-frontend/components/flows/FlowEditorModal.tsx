...
import { X, Save, Code, Eye, Workflow, Trash2, Smartphone, Battery, Wifi, Signal, ChevronLeft, Plus, CheckCircle2, XCircle } from 'lucide-react';
        ...
        },
        {
            ...
            "layout": {
                ...
                "children": [
                    ...
                    { "type": "TextBody", "text": "Thank you! We have received your booking request." },
                    {
                        "type": "Footer",
                        "label": "Done",
                        "on-click-action": {
...
export default function FlowEditorModal({ isOpen, onClose, flow, onSave }: FlowEditorModalProps) {
    ...
    const parseJSONToScreens = (json: any): FlowScreen[] => {
        ...
        return json.screens.map((s: any) => ({
            ...
            children: parseChildrenToComponents(s.layout?.children || [], json.routing_model, s.id)
        }));
    };

    const parseChildrenToComponents = (children: any[], routingModel: any = {}, currentScreenId: string = ''): FlowComponent[] => {
        return children.map((child: any, index: number) => {
            ...
            if (child.type === 'TextHeading') {
                ...
                type = 'TextInput';
                data = { label: child.label, name: child.name, required: child.required, inputType: child['input-type'] };
            } else if (child.type === 'Dropdown') {
                type = 'Dropdown';
                data = { label: child.label, name: child.name, required: child.required, options: child['data-source'] };
                ...
                type = 'DatePicker';
                data = { label: child.label, name: child.name, required: child.required };
                ...
                return parseChildrenToComponents(child.children || [], routingModel, currentScreenId);
            }
    ...
    const validateJSON = (jsonString: string) => {
        try {
            ...
            if (!parsed.version || !parsed.screens || !Array.isArray(parsed.screens)) {
    ...
    const [activeTab, setActiveTab] = useState<'builder' | 'json' | 'validation'>('builder'); // Default to builder
    ...
    // --- NEW BUILDER STATE ---
    type FlowComponentType = 'TextHeading' | 'TextBody' | 'TextInput' | 'Dropdown' | 'RadioButtons' | 'CheckboxGroup' | 'DatePicker' | 'Footer';
    ...
    const [screens, setScreens] = useState<FlowScreen[]>([
        {
            ...
            children: [
                ...
                { id: 'c2', type: 'TextBody', data: { text: 'Please fill out the form below.' } },
        ...
        },
        {
            ...
            terminal: false,
            children: [
                { id: 'c4', type: 'TextInput', data: { label: 'Full Name', name: 'name', required: true, inputType: 'text' } },
                { id: 'c5', type: 'Footer', data: { label: 'Submit', onNext: 'SUCCESS' } }
            ]
        },
        {
            id: 'SUCCESS',
            ...
            children: [
                ...
                { id: 'c7', type: 'TextBody', data: { text: 'Your response has been saved.' } },
    ...
    const generateJSONFromScreens = () => {
        const screensJson = screens.map(screen => {
            const childrenJson = screen.children.map(comp => {
                switch (comp.type) {
                    case 'TextHeading': return { type: 'TextHeading', text: comp.data.text };
                    case 'TextBody': return { type: 'TextBody', text: comp.data.text };
                    case 'TextInput': return { type: 'TextInput', name: comp.data.name, label: comp.data.label, required: comp.data.required, 'input-type': comp.data.inputType || 'text' };
                    case 'Dropdown': return { type: 'Dropdown', name: comp.data.name, label: comp.data.label, required: comp.data.required, 'data-source': comp.data.options || [] };
                    case 'RadioButtons': return { type: 'RadioButtons', name: comp.data.name, label: comp.data.label, required: comp.data.required, 'data-source': comp.data.options || [] };
                    case 'CheckboxGroup': return { type: 'CheckboxGroup', name: comp.data.name, label: comp.data.label, required: comp.data.required, 'data-source': comp.data.options || [] };
                    case 'DatePicker': return { type: 'DatePicker', name: comp.data.name, label: comp.data.label, required: comp.data.required };
                    case 'Footer':
                        ...
                        if (actionType === 'complete') {
                            ...
                            const formFields = screen.children.filter(c => ['TextInput', 'Dropdown', 'RadioButtons', 'CheckboxGroup', 'DatePicker'].includes(c.type));
                            const payload: any = {};
                            formFields.forEach(f => { payload[f.data.name] = `\${form.${f.data.name}}`; });
                            ...
                            if (nextId) {
                                action = { name: 'navigate', next: { type: 'screen', name: nextId }, payload: {} };
                            } else {
                        ...
                        return { type: 'Footer', label: comp.data.label, 'on-click-action': action };
                    default: return null;
                }
            }).filter(Boolean);
        ...
        screens.forEach(s => {
            ...
            if (footer) {
                ...
                if ((actionType === 'navigate' || actionType === 'data_exchange') && nextId) {
    ...
    const addScreen = () => {
        ...
        setScreens([...screens, { id, title: 'New Screen', terminal: false, children: [] }]);
        setActiveScreenId(id);
    };

    const deleteScreen = (id: string) => {
        if (screens.length <= 1) return toast.error("Cannot delete the only screen");
    ...
    const updateComponent = (compId: string, data: any) => {
        setScreens(screens.map(s => {
            if (s.id === activeScreenId) {
                return {
                    ...s,
                    children: s.children.map(c => c.id === compId ? { ...c, data: { ...c.data, ...data } } : c)
                };
            }
            return s;
        }));
    ...
    const selectedComponentContext = activeScreenContext?.children.find(c => c.id === selectedComponentId);
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
                    ...
                    >
                        ...
                        <div className="px-4 py-2 border-b border-border bg-muted/10">
                            <label className="block text-xs font-semibold text-muted-foreground mb-1">Endpoint URI (Required)</label>
                            <input
                                type="url"
                            ...
                            <div className="flex justify-end mt-1">
                                <button
                                    onClick={() => setFormData({ ...formData, endpointUri: `${window.location.protocol}//${window.location.host}/api/whatsapp/flows/endpoint` })}
                        ...
                        <div className="flex border-b border-border bg-muted/30 px-4">
                            ...
                            <button
                                onClick={() => { setActiveTab('json'); generateJSONFromScreens(); }} // Sync on switch
                            ...
                            >
                                Validation
                                {validationErrors.length > 0 && <span className="ml-2 bg-red-100 text-red-600 px-1.5 py-0.5 rounded-full text-[10px]">{validationErrors.length}</span>}
                            </button>
                        </div>
                        ...
                        {activeTab === 'validation' && (
                            <div className="flex-1 overflow-y-auto p-4 bg-muted/5">
                                <div className="flex justify-between items-center mb-4">
                                    <h3 className="font-semibold text-sm">Meta Validation Errors</h3>
                                ...
                                {validationErrors.length === 0 ? (
                                    <div className="flex flex-col items-center justify-center h-40 text-muted-foreground bg-green-500/5 rounded-lg border border-green-500/20">
                                        ...
                                        <p className="text-sm font-medium text-green-700">No Validation Errors Found</p>
                                        <p className="text-xs">Your Flow JSON structure looks good to Meta.</p>
                                    </div>
                                ) : (
                                    <div className="space-y-3">
                                        {validationErrors.map((err: any, idx) => (
                                            <div key={idx} className="bg-red-50 border border-red-200 p-3 rounded-lg flex gap-3 text-sm text-red-900">
                                                <XCircle className="w-5 h-5 text-red-500 shrink-0 mt-0.5" />
                                                <div>
                                                    <p className="font-semibold">{err.error_code} - {err.error_title || 'Validation Error'}</p>
                                                    <p className="text-red-700 mt-1">{err.error}</p>
                                                    <p className="text-xs text-red-500 mt-2 font-mono bg-white/50 p-1 rounded inline-block">{err.details}</p>
                                                </div>
                                            </div>
                                        ))}
                                    </div>
                        ...
                        <div className="flex-1 overflow-hidden flex">
                            {/* VISUAL BUILDER */}
                            {activeTab === 'builder' && (<div className="flex bg-muted/5 h-full overflow-hidden w-full">

                                {/* 1. LEFT SIDEBAR: SCREEN MANAGER */}
                                <div className="w-64 bg-card border-r border-border flex flex-col shrink-0">
                                    <div className="p-4 border-b border-border bg-muted/20">
                                        <h3 className="font-semibold text-xs uppercase tracking-wider text-muted-foreground">Screens</h3>
                                    </div>
                                    <div className="flex-1 overflow-y-auto p-2 space-y-1">
                                        {screens.map(s => (
                                            <div
                                                key={s.id}
                                                onClick={() => { setActiveScreenId(s.id); setSelectedComponentId(null); }}
                                            ...
                                            >
                                                ...
                                                {screens.length > 1 && (
                                                    <Button
                                                        ...
                                                        onClick={(e) => { e.stopPropagation(); deleteScreen(s.id); }}
                                                    >
                                    ...
                                    <div className="p-3 border-t border-border">
                                        <Button variant="outline" className="w-full text-xs" onClick={addScreen}>+ Add Screen</Button>
                                    </div>
                                </div>
                                ...
                                <div className="flex-1 overflow-y-auto bg-muted/10 flex flex-col items-center p-4 relative">
                                    ...
                                    <div className="w-[320px] h-[640px] bg-background border-[6px] border-zinc-800 rounded-[3rem] shadow-2xl relative flex flex-col shrink-0 scale-90 origin-center overflow-hidden my-auto">
                                        ...
                                        <div className="bg-[#008069] text-white px-4 py-2 flex items-center gap-3 shadow-md z-10 shrink-0 select-none">
                                            <div className="w-8 h-8 rounded-full bg-white/20 flex items-center justify-center text-xs font-bold">A</div>
                                            <div className="flex-1">
                                                <div className="text-sm font-semibold leading-none">Flow Builder</div>
                                                <div className="text-[10px] opacity-80 mt-0.5">WhatsApp Flows</div>
                                            </div>
                                        </div>
                                        ...
                                        <div className="flex-1 bg-[#efeae2] p-3 overflow-y-auto relative scrollbar-hide">
                                            ...
                                            <div className="bg-white rounded-lg shadow-sm min-h-[300px] flex flex-col relative overflow-hidden text-zinc-900">
                                                ...
                                                <div className="h-10 border-b flex items-center justify-between px-3 shrink-0 bg-white z-10 sticky top-0">
                                                    ...
                                                    <span className="font-semibold text-xs truncate max-w-[150px] text-zinc-900">{activeScreenContext?.title}</span>
                                                    <div className="w-4" />
                                                </div>
                                                ...
                                                <div className="flex-1 p-4 space-y-4 overflow-y-auto">
                                                    {activeScreenContext?.children.length === 0 && (
                                                        <div className="text-center py-10 text-xs text-muted-foreground border-2 border-dashed rounded-lg">
                                                            Empty Screen <br /> Add components from right panel
                                                        </div>
                                                    )}
                                                    ...
                                                    {activeScreenContext?.children.map((comp) => (
                                                        ...
                                                        >
                                                            {/* Render Component */}
                                                            {comp.type === 'TextHeading' && <h1 className="text-lg font-bold text-zinc-900">{comp.data.text}</h1>}
                                                            {comp.type === 'TextBody' && <p className="text-sm text-zinc-500 whitespace-pre-wrap">{comp.data.text}</p>}

                                                            {(comp.type === 'TextInput' || comp.type === 'DatePicker') && (
                                                                <div className="space-y-1">
                                                                    <label className="text-xs font-bold text-zinc-500 flex gap-0.5">
                                                                        {comp.data.label} {comp.data.required && <span className="text-red-500">*</span>}
                                                                    </label>
                                                                    <div className="h-10 w-full bg-transparent border border-zinc-300 rounded-lg px-3 flex items-center text-sm text-zinc-400">
                                                                        {comp.type === 'TextInput' ? (
                                                                            <span className="truncate">{comp.data.text || `Enter ${comp.data.label}`}</span>
                                                                        ) : 'DD/MM/YYYY'}
                                                                    </div>
                                                                </div>
                                                            )}

                                                            {(comp.type === 'Dropdown' || comp.type === 'RadioButtons' || comp.type === 'CheckboxGroup') && (
                                                                <div className="space-y-1">
                                                                    <label className="text-xs font-bold text-zinc-500 flex gap-0.5">
                                                                        {comp.data.label} {comp.data.required && <span className="text-red-500">*</span>}
                                                                    </label>
                                                                        ...
                                                                        <div className="space-y-2 pt-1">
                                                                            {comp.data.options?.map((opt: any) => (
                                                                                <div key={opt.id} className="flex items-center gap-2">
                                                                                    <div className={`w-5 h-5 border border-zinc-400 ${comp.type === 'RadioButtons' ? 'rounded-full' : 'rounded-md'} flex items-center justify-center`}></div>
                                                                                    <span className="text-sm text-zinc-700">{opt.title || opt.id}</span>
                                                                                </div>
                                                                            ))}
                                                                            {(!comp.data.options || comp.data.options.length === 0) && <span className="text-[10px] italic text-zinc-400">No options added</span>}
                                                                        </div>
                                                                    )}
                                                                </div>
                                                            )}
                                ...
                                <div className="w-96 bg-card border-l border-border flex flex-col shrink-0 overflow-hidden">
                                    ...
                                    <div className="flex border-b text-xs font-semibold">
                                        <button
                                            ...
                                            onClick={() => { setSelectedComponentId(null); setActiveRightTab('components'); }}
                                        >
                                            Components
                                        </button>
                                        <button
                                    ...
                                    <div className="flex-1 overflow-y-auto p-4">
                                        ...
                                        {activeRightTab === 'components' ? (
                                            <div className="space-y-6">
                                                ...
                                                <div>
                                                    <h4 className="text-xs font-bold uppercase text-muted-foreground mb-3">Layout Elements</h4>
                                                    <div className="grid grid-cols-2 gap-2">
                                                        <Button variant="outline" size="sm" className="justify-start text-xs h-8" onClick={() => addComponent('TextHeading')}>Heading</Button>
                                                        <Button variant="outline" size="sm" className="justify-start text-xs h-8" onClick={() => addComponent('TextBody')}>Body Text</Button>
                                                        <Button variant="outline" size="sm" className="justify-start text-xs h-8" onClick={() => addComponent('Footer')}>Footer Button</Button>
                                                    </div>
                                                </div>

                                                <div>
                                                    <h4 className="text-xs font-bold uppercase text-muted-foreground mb-3">Form Inputs</h4>
                                                    <div className="grid grid-cols-2 gap-2">
                                                        <Button variant="outline" size="sm" className="justify-start text-xs h-8" onClick={() => addComponent('TextInput')}>Text Input</Button>
                                                        <Button variant="outline" size="sm" className="justify-start text-xs h-8" onClick={() => addComponent('Dropdown')}>Dropdown</Button>
                                                        <Button variant="outline" size="sm" className="justify-start text-xs h-8" onClick={() => addComponent('RadioButtons')}>Radio Group</Button>
                                                        <Button variant="outline" size="sm" className="justify-start text-xs h-8" onClick={() => addComponent('CheckboxGroup')}>Checkbox</Button>
                                                        <Button variant="outline" size="sm" className="justify-start text-xs h-8" onClick={() => addComponent('DatePicker')}>Date Picker</Button>
                                                    </div>
                                                </div>
                                            </div>
                                        ) : (
                                                ...
                                                {!selectedComponentId ? (
                                                    ...
                                                    <div className="space-y-3">
                                                        <div className="flex items-center justify-between mb-2">
                                                            <h4 className="text-xs font-bold uppercase text-muted-foreground">Screen Properties</h4>
                                                        </div>
                                                        <div className="p-3 bg-muted/20 rounded-lg border space-y-3">
                                                            <div>
                                                                ...
                                                                <input className="w-full text-xs font-mono bg-background border px-2 py-1.5 rounded disabled:opacity-50" value={activeScreenContext?.id} disabled />
                                                            </div>
                                                            <div>
                                                            ...
                                                            <div className="pt-2">
                                                                <label className="text-[10px] uppercase font-bold text-muted-foreground">Terminal Screen?</label>
                                                                <div className="flex items-center gap-2 mt-1">
                                                                    <input
                                                                        type="checkbox"
                                                                        checked={activeScreenContext?.terminal}
                                                                        onChange={(e) => setScreens(screens.map(s => s.id === activeScreenId ? { ...s, terminal: e.target.checked } : s))}
                                                                    />
                                                                    <span className="text-xs text-muted-foreground">End Navigation Logic Here</span>
                                                                </div>
                                                            </div>
                                                        </div>
                                                        <div className="text-[10px] text-muted-foreground p-2">
                                                            Select a component in the simulator to edit its properties.
                                                        </div>
                                                    </div>
                                                ) : (
                                                    /* COMPONENT PROPERTIES */
                                                    <>
                                                        <div className="flex items-center justify-between mb-2">
                                                            <Button variant="ghost" size="sm" className="-ml-2 text-xs text-muted-foreground" onClick={() => { setSelectedComponentId(null); setActiveRightTab('components'); }}>
                                                                <ChevronLeft className="w-4 h-4 mr-1" /> Back
                                                            </Button>
                                                        </div>
                                                        <div className="flex items-center justify-between">
                                                            <span className="text-xs font-bold bg-primary/10 text-primary px-2 py-1 rounded">{selectedComponentContext?.type}</span>
                                                        </div>
                                                        ...
                                                        <div className="space-y-4">
                                                            ...
                                                            {(selectedComponentContext?.type === 'TextHeading' || selectedComponentContext?.type === 'TextBody') && (
                                                                <div>
                                                                    <label className="text-[10px] uppercase font-bold text-muted-foreground">Text Content</label>
                                                                    <textarea
                                                                        className="w-full text-xs bg-background border px-2 py-1.5 rounded min-h-[80px]"
                                                                        value={selectedComponentContext?.data.text || ''}
                                                                        onChange={(e) => selectedComponentId && updateComponent(selectedComponentId, { text: e.target.value })}
                                                                    />
                                                                </div>
                                                            )}

                                                            {/* 2. Input Fields: Label & Name & Required */}
                                                            {['TextInput', 'Dropdown', 'RadioButtons', 'CheckboxGroup', 'DatePicker', 'Footer'].includes(selectedComponentContext?.type || '') && (
                                                                <div className="space-y-3">
                                                                    <div>
                                                                        <label className="text-[10px] uppercase font-bold text-muted-foreground">Label / Button Text</label>
                                                                        <input
                                                                            type="text"
                                                                            className="w-full text-xs bg-background border px-2 py-1.5 rounded"
                                                                    ...
                                                                    {selectedComponentContext?.type !== 'Footer' && (
                                                                        <>
                                                                            <div>
                                                                                <label className="text-[10px] uppercase font-bold text-muted-foreground">Field Name (ID)</label>
                                                                                <input
                                                                                    type="text"
                                                                            ...
                                                                            <div className="flex items-center gap-2">
                                                                                ...
                                                                                />
                                                                                <label htmlFor="req-check" className="text-xs">Required Field</label>
                                                                            </div>
                                                                        </>
                                                                    )}
                                                                </div>
                                                            ...
                                                            {(selectedComponentContext?.type === 'Dropdown' || selectedComponentContext?.type === 'RadioButtons' || selectedComponentContext?.type === 'CheckboxGroup') && (
                                                                <div>
                                                                    <div className="flex items-center justify-between">
                                                                        <label className="text-[10px] uppercase font-bold text-muted-foreground">Options List</label>
                                                                        <Button
                                                                            variant="outline" size="sm" className="h-5 text-[10px] px-2"
                                                                            onClick={() => {
                                                                    ...
                                                                    <div className="space-y-2 max-h-[200px] overflow-y-auto pr-1">
                                                                        {(selectedComponentContext?.data.options || []).map((opt: any, idx: number) => (
                                                                            <div key={idx} className="flex items-center gap-2 group">
                                                                                ...
                                                                                <Button
                                                                                    ...
                                                                                    onClick={() => {
                                                                                        const newOpts = selectedComponentContext.data.options.filter((_: any, i: number) => i !== idx);
                                                                                        updateComponent(selectedComponentContext.id, { options: newOpts });
                                                                                    }}
                                                                                >
                                                                                    <X className="w-3 h-3" />
                                                                        ...
                                                                        {(!selectedComponentContext?.data.options || selectedComponentContext.data.options.length === 0) && (
                                                            ...
                                                            <div>
                                                                <div className="mb-2">
                                                                    ...
                                                                    <select
                                                                        className="w-full text-xs bg-background border px-2 py-1.5 rounded"
                                                                        value={selectedComponentContext?.data.actionType || 'navigate'}
                                                                ...
                                                                {(selectedComponentContext?.data.actionType === 'navigate' || selectedComponentContext?.data.actionType === 'data_exchange') && (
                                                                    <div>
                                                                        <label className="text-[10px] uppercase font-bold text-muted-foreground">Target Screen</label>
                                                                        <select
                                                                            className="w-full text-xs bg-background border px-2 py-1.5 rounded"
                                                                        ...
                                                                        >
                                                                            ...
                                                                            {screens
                                                                                .filter(s => s.id !== activeScreenId) // Prevent self-linking loop
                                                                                .map(s => (
                                ...
                                <div className="flex-1 relative">
                                    ...
                                    <div className="absolute top-2 right-4 flex gap-2">
                                        <Button
                                            ...
                                            variant="secondary"
                                            onClick={() => {
                                                setFormData({ ...formData, flowJson: JSON.stringify(SAMPLE_FLOW_JSON, null, 2) });
                                            }}
