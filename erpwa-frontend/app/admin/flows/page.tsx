...
export default function FlowsPage() {
    ...
    const handleSetupEncryption = () => {
        toast('Generate new Encryption Keys?', {
            ...
            action: {
                ...
                onClick: async () => {
                    try {
                        ...
                        toast.success(response.data.message || 'Encryption keys set up successfully!');
                    } catch (error: any) {
                        console.error('Error setting up keys:', error);
                        toast.error(error.response?.data?.message || 'Failed to setup encryption keys');
                    } finally {
                ...
                onClick: async () => {
                    try {
                        ...
                        if (validationErrors && validationErrors.length > 0) {
                            const errorDetails = validationErrors.map((e: any) => e.error).join('\n');
    ...
    const filteredFlows = flows.filter(flow => {
        ...
        const matchesStatus = statusFilter === 'all' || flow.status.toLowerCase() === statusFilter.toLowerCase();
    ...
    const getStatusBadge = (status: Flow['status']) => {
        const statusConfig: Record<Flow['status'], { color: string; icon: any }> = {
            ...
            PUBLISHED: { color: 'bg-green-100 text-green-700 dark:bg-green-900 dark:text-green-300', icon: CheckCircle2 },
            DEPRECATED: { color: 'bg-red-100 text-red-700 dark:bg-red-900 dark:text-red-300', icon: XCircle }
        };
